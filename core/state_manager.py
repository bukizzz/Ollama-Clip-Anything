...
class StateManager:
    _instance = None
    _initialized = False
    _lock = threading.Lock() # Add a lock for thread-safe operations
    ...
    def _load_state_file(self) -> Dict[str, Any]:
        with self._lock: # Acquire lock before loading
    ...
    def update_state_file(self, new_state_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self._lock: # Acquire lock before updating and saving
    ...
    def get_state(self) -> Dict[str, Any]:
        with self._lock: # Acquire lock before getting state
            return self.current_state

    def delete_state_file(self):
        """Deletes the state file."""
        with self._lock: # Acquire lock before deleting
            if os.path.exists(self.state_file_path):
                try:
...
def set_stage_status(stage_name: str, status: str, details: Dict[str, Any] = None):
    ...
    # Get timestamp, handle potential file not found gracefully
    timestamp: float = 0.0
    if os.path.exists(state_manager.state_file_path):
        try:
            timestamp = float(os.path.getmtime(state_manager.state_file_path))
        except Exception as e:
            logging.warning(f"Could not get modification time for state file: {e}. Using 0.0 as timestamp.")
            timestamp = 0.0
    
    current_pipeline_stages[stage_name] = {'status': status, 'timestamp': timestamp}
    if details:
