...
class VideoProductionAgent(Agent):
    ...
    def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # --- Pre-flight Check ---
        # If created clips already exist in the context, skip this stage.
        if context.get('created_clips') and context.get('pipeline_stages', {}).get(stage_name) == 'complete':
            print(f"✅ Skipping {stage_name}: Video production already complete.")
            return context
        ...
        try:
            ...
            # Retrieve zoom_events generated by DynamicEditingAgent
            zoom_events = context.get("zoom_events", [])
            ...
            # Add video_encoder to processing_settings
            processing_settings['video_encoder'] = self.config.get('video_encoder')
            ...
            created_clips, processing_report = video_editing.batch_process_with_analysis(
                ...
                zoom_events=zoom_events # Pass pre-calculated zoom events
            )
            ...
            for clip in created_clips:
                ...
                subtitle_path = generate_subtitles_efficiently(
                    ...
                    clip_name,
                    video_info.get('width'), # Pass video_width
                    video_info.get('height')
                )
                if subtitle_path: # Only add if a valid path is returned
